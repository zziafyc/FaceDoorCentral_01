...
public class IdentifyActivity extends Activity {
    ...
    private IdentityListener mSearchListener = new IdentityListener() {
        ...
        public void onError(SpeechError error) {
            ...
            delayedFinish(NO_FACE);
           /* mSpeaker.startSpeaking(error.getErrorDescription(), new SynthesizerListener() {
                @Override
    ...
    private void identifyResult(IdentityResult result) {
        ...
        try {
            ...
            if (ErrorCode.SUCCESS == resultJson.getInt("ret")) {
                JSONObject candidateOne = resultJson.getJSONObject("ifv_result").getJSONArray("candidates")
                        .getJSONObject(0);
                if (candidateOne == null) {
                    return;
                }
                String decision = candidateOne.getString("decision");
                double score = candidateOne.getDouble("score");
                mScore.setText("" + score);
                if ("accepted".equals(decision) && score > 88) {
                    authId = candidateOne.getString("user");
                    Observable.create(new OnSubscribe<Integer>() {
                        @Override
                        public void call(Subscriber<? super Integer> arg0) {
                            //将语音的初始化放在rxjava的耗时操作中
                            initVerifier();
                ...
                mSpeaker.startSpeaking("对不起，鉴别失败", null);
